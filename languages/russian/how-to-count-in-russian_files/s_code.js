...
s_account = ( typeof s_account == 'undefined' ) ? "wileydumcom" : s_account; 
...
s.prop1 = document.location.pathname;
s.prop2 = "Desktop";
...
function s_doPlugins(s) {
		...
		s.eVar2=(s.getQueryParam('cmed').length > 0) ? s.getQueryParam('cmed') : getHashQueryParam('cm');
		s.eVar3=(s.getQueryParam('csource').length > 0) ? s.getQueryParam('csource') : getHashQueryParam('cs');
		s.eVar4=(s.getQueryParam('cunit').length > 0) ? s.getQueryParam('cunit') : getHashQueryParam('cu');
		s.campaign='D=v1+"|"+v2+"|"+v3+"|"+v4';
		...
		if(typeof(pageVars['effectiveDate.day'])!=='undefined'){
			s.prop75 = pageVars['effectiveDate.day'] + "|" + pageVars['effectiveDate.hours'] + "|" + (Math.floor(pageVars['effectiveDate.minutes']/15)*15);
			s.eVar7=s.prop75;
		}
		...
		if  ( window.pageVars ){
			...
			/* eCommerce*/
			if(pageVars['navArea'] == "DUMMIES2_STORE" && pageVars['product.productCode']){
				s.prop30 =  pageVars['product.productCode'];
				s.eVar20 = "D=c30";
				s.products = ";" +  pageVars['product.productCode'];
				if(pageVars['pVOTypeNm']=="Product" && pageVars.pageCd=='undefined') s.prop7="Description";
				//event serialization using JSESSIONID to prevent duplicate prodview events
				//total id must be 20 chars or less
			...
			if(pageVars['article.id']){
				...
				if (pageVars['article.articleType']=='VIDEO') s.prop42="VIDEO";
					...
					else s.prop42="STEP_BY_STEP_SLIDESHOW";
				}
				else{
					s.prop42="UNKNOWN";
				}
